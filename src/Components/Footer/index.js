...
import { FaLongArrowAltRight,FaFacebookF,FaTwitter,FaBehanceSquare,FaDribbbleSquare } from "react-icons/fa";
  ...
  <div className="flex flex-wrap   mt-5 md:ml-10 md:flex-row md:flex-wrap">
    {images.map((image) => (
      <div key={image.id} className="w-1/4 p-1  md:w-auto">
        <img src={image.src} alt={image.alt} className="md:h-[60px] md:w-[60px] md:object-cover" />
      </div>
    ))}
  </div>
</div>
     ...
     <p className="text-base w-full bg-slate-950  text-center mr-10  text-white flex">
          <span>Copyright ©2023 All rights reserved | This template is made with  by Sher</span>
        </p>
     </div>
     </div>
  );
