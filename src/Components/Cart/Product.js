  ...
  return (
    <div className="flex md:mt-[38rem] md:w-auto w-[20rem]  mt-[26rem] mb-10 bg-white flex-col">
      <div className="md:flex md:w-auto w-[20rem] hidden text-lg  md:flex-row bg-white">
        <h1 className="text-gray-600 pl-20">title</h1>
        <h1 className="text-gray-600 pl-[40rem]">Price</h1>
      ...
      <div className="flex flex-col md:flex-row md:h-auto h-[11rem] bg-white">
        <div className="flex flex-row w-[20rem] md:w-auto  ">
          <img
            src={Logo}
            alt="pic"
            className={`md:h-[300px] h-[200px] w-[200px] md:w-[300px] ${
          ...
          <div className="flex flex-col md:flex-1">
            <div className="flex flex-rows">
              <div>
                <h1 className="md:text-2xl mt-20 md:mt-32 md:ml-16">{title}</h1>
              </div>
              {!isSmallerScreen && (
                <>
                  <h1 className="md:text-xl  md:mt-36 md:ml-80">${price}</h1>
                  <div className="flex md:flex-row flex-col relative md:ml-0 ml-[-28]">
                    <input
                      type="number"
                      id="numbers"
                      name="numbers"
                      min="0"
                      max="100"
                      className="w-40 md:w-40 md:pl-20 text-xl md:mt-32 px-4 py-4 md:ml-20 border-2 border-gray-300 focus:outline-none"
                    />
                    <span className="absolute md:top-[75%] top-[10%] left-0  md:left-24  md:mr-4 text-xl font-bold text-gray-500">
                      Qty
                    </span>
                  </div>
                  <div>
                    <h1 className="text-xl mt-1 md:ml-32   md:mt-36">
                      ${Price}
                    </h1>
                  </div>
                </>
            ...
            <div>
              <p
                className={`md:text-lg mb-20 md:ml-16 ${
                  isSmallerScreen ? "md:flex-1" : ""
                }`}
              >
                {description}
